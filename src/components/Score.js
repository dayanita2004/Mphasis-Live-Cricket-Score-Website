...
// 👉 get all balls of this over
const overBalls = match.commentary?.filter(
  c => c.over === currentOver
) || [];
 
// 👉 build proper display
let legalCount = 0;
const displayBalls = [];
 
for (let b of overBalls) {
  displayBalls.push(b.value);
...
<p>
  Overs: Overs: {match.overs.over}.{match.overs.ball}
</p>                      <p>
  <strong>Current Over: </strong>
